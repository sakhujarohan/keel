...
- `[x]` — 12 tests asserting M4's split literally (thirteen rules, the 9/4 block-warn boundary, the table frozen) and M3's exit code in all four combinations.
...
- `[x]` — context loads once and is deep-frozen; commits load only when a rule asks.
...
- `[x]` — KC-01/04/05/06. KC-05 caught a real parser bug: `R8 (M1, M2)` was read as a token called `R8 (M1`, so delivered requirements looked orphaned.
...
- `[x]` — KC-02/03/09, verified against gates sealed by the real tool rather than fixtures. KC-09 adopts the three orphaned ledger diagnostics.
...
- `[x]` — KC-07/08/10/11/12/13. KC-11 caught a real rule bug: it rejected mandate IDs, though work legitimately traces to them.
...
- `[x]` — engine, hook path, A6 handling, determinism. **Dogfooded against this repository — see below.**
...
## Dogfood: the engine against this repository (2026-07-21)

Run before sealing anything, the engine reported **19 blocking, 24 warnings**. Two of those
findings were bugs in my own rules and are fixed; the rest were true. The interesting part is what
happened next.

| | before sealing | after sealing | why |
|---|---|---|---|
| KC-02 artifact before its gate | 11 | **0** | every gate in this run had been signed in conversation, with nothing to back it. Sealing them with `prepareSeal`/`commitSeal` made the claims real |
| KC-10 STATUS projection | 5 | **0** | same cause: `STATUS.md` claimed sign-offs the ledger could not support |
| KC-05 / KC-11 | 17 | **0** | rule bugs, fixed |
| KC-06 features without an LLD | 4 | **4** | true: `state-projection`, `scaffold`, `adapters` and `upgrade` are designed but not yet built |
| KC-13 unattributed commits | 6 | **6** | true: the v1-era commits predate D6 and cannot be attributed without rewriting history |

The tool's first real act was to catch its own project doing exactly what it was built to prevent —
gates asserted in a chat transcript with no record behind them. `.keel/gates.jsonl` now holds nine
sealed gates for this run.
