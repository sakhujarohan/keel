...
|-------|-----------|-----------|
| 0 Kickoff | `context.md` | One line: mode, budget, profile (usually `prototype`). |
| 1 Requirements | Full EARS list + NFRs + out-of-scope | The 5–8 requirements that matter, in plain testable sentences, **plus an explicit out-of-scope list**. Still gate at G1. |
...
| 5 Tasks | Dependency waves | A short ordered checklist. |
| 6 Build | Full test depth per profile | Implement the core path; test the critical behavior; note what you'd test with more time. |
| 7 Review | Full checklist | Speak the top risks (concurrency, correctness, failure handling) and the known limitations. |
...
- **~20% — Requirements + clarify (Phase 1, G1).** The cheapest minutes you'll spend. Lock scope before anything else.
- **~25% — HLD + stack (Phases 2–3, G2/G3).** Shape and tools, confirmed.
- **~10% — LLD for the first feature (Phase 4, G4).**
- **~40% — Build the core path (Phase 6).**
- **~5% — Surface risks and limitations (Phase 7).**
